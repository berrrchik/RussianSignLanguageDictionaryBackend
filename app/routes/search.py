...
from app.utils.similarity import find_similar_signs
...
@require_json
@handle_db_errors('генерации embeddings')
...
@require_json
@handle_db_errors('поиска похожих жестов')
def search_similar() -> Tuple[Dict[str, Any], int]:
    """
    ...
    responses:
      ...
      503:
        description: Модель недоступна
      500:
        description: Ошибка генерации или поиска
    """
    data = request.get_json()
    ...
    query_embedding = EmbeddingsService.generate_for_text(text)
    if not query_embedding:
        return error_response('GENERATION_FAILED', 'Не удалось сгенерировать embeddings', 500)
    
    # Загрузка всех жестов с embeddings
    signs = db.session.query(Sign).filter(Sign.embeddings.isnot(None)).all()
    
    # Подготовка данных для поиска
    signs_with_embeddings = [
        (sign.id, sign.word, sign.embeddings)
        for sign in signs
        if sign.embeddings is not None
    ]
    
    # Поиск похожих жестов
    similar_results = find_similar_signs(
        query_embedding=query_embedding,
        signs_with_embeddings=signs_with_embeddings,
        limit=limit,
    ...
    results = [
        ...
        for sign_id, word, similarity in similar_results
    ]
